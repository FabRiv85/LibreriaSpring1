...
public class CategoriaControllerTestIntegracion {
    ...
    public void testGetCategoria()throws Exception{
        Categoria categoria = new Categoria(1,"Terror","Trauma de la programacion");
        Mockito.when(categoriaService.findAll()).thenReturn(List.of(categoria));
        mockMvc.perform(get("/api/categorias"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].categoria").value("Terror"));
    ...
    public void testPostCategoria() throws Exception {
        ...
        mockMvc.perform(post("/api/categorias")
            ...
            .content(objectMapper.writeValueAsString(categoria)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.categoria").value("Terror"));
    }
    @Test
